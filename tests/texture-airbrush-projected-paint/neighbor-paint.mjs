...
test("airbrush WebGL brush shader discards fragments outside an active neighbor mask", () => {
  ...
  assert.match(material.fragmentShader, /Do not feed it a[\s\S]*screen-neighborhood depth\/normal mask/);
  assert.match(material.fragmentShader, /neighboring samples can punch triangle-shaped holes/);
  // DO NOT PAINT ON NON CAMERA FACING SIDES.
  ...
  assert.match(material.fragmentShader, /float radiusScale = clamp\(radiusPixels \/ 32\.0, 0\.0, 1\.0\)/);
  assert.match(material.fragmentShader, /return visibleFacingNormalThreshold \+ mix\(0\.28, 0\.64, radiusScale\)/);
  assert.match(material.fragmentShader, /Larger brushes need a broader soft/);
  ...
  assert.match(material.fragmentShader, /float visibleSurfaceGeometricCutoffCoverage/);
  assert.match(material.fragmentShader, /visibleFacingNormalThreshold \+ 0\.075/);
  ...
  assert.match(material.fragmentShader, /float smoothNormalCoverage = visibleSurfaceGrazingAngleCoverage\(paintFadeViewNormal\)/);
  assert.match(material.fragmentShader, /float smoothAirbrushCoverage = mix\(0\.9, 1\.0, smoothNormalCoverage\)/);
  assert.match(material.fragmentShader, /float geometricCutoffCoverage = visibleSurfaceGeometricCutoffCoverage\(paintGateViewNormal\)/);
  assert.match(material.fragmentShader, /return smoothAirbrushCoverage \* geometricCutoffCoverage/);
  assert.match(material.fragmentShader, /float smoothNormalGrazingEdgeAmount = visibleSurfaceGrazingEdgeAmount\(paintFadeViewNormal\)/);
  assert.match(material.fragmentShader, /float geometricCutoffEdgeAmount = 1\.0 - visibleSurfaceGeometricCutoffCoverage\(paintGateViewNormal\)/);
  assert.match(material.fragmentShader, /return max\(smoothNormalGrazingEdgeAmount, geometricCutoffEdgeAmount\)/);
  assert.match(material.fragmentShader, /tiny zero-at-cutoff cap/);
  assert.match(material.fragmentShader, /must not get dark enough to reveal triangle facets/);
  ...
  assert.match(material.fragmentShader, /The hard discard below still owns hidden\/back rejection/);
  ...
  assert.match(material.fragmentShader, /float grazingFeatherEnd = visibleSurfaceGrazingFeatherEnd\(\)/);
  assert.match(material.fragmentShader, /row of triangle teeth/);
  ...
  assert.match(material.fragmentShader, /geometric cutoff cap below enforces exact zero/);
  assert.match(material.fragmentShader, /return pow\(angleCoverage, 0\.35\)/);
  ...
  assert.match(material.fragmentShader, /float centerGrazingEdgeAmount = visibleSurfaceSoftEdgeAmount\(paintFadeViewNormal, paintGateViewNormal\) \* edgeSoftness/);
  ...
  assert.match(material.fragmentShader, /Do not feed it a[\s\S]*screen-neighborhood depth\/normal mask/);
  assert.match(material.fragmentShader, /Do not floor the geometric cutoff/);
  assert.match(material.fragmentShader, /paint leaking onto the non-camera-facing side/);
  assert.match(material.fragmentShader, /rejected hidden\/back fragments still discard/);
  ...
  assert.match(material.fragmentShader, /A nonzero floor at the[\s\S]*cutoff looks like paint leaking/);
