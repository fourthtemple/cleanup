...
test("airbrush WebGL brush shader discards fragments outside an active neighbor mask", () => {
  ...
  assert.match(material.fragmentShader, /visibleFacingNormalThreshold \+ 0\.04/);
  assert.match(material.fragmentShader, /visibleFacingNormalThreshold \+ 0\.36/);
  assert.match(material.fragmentShader, /return mix\(normalMatchThreshold, 0\.55, frontFacingRelax\)/);
  assert.match(material.fragmentShader, /The first tiny band above the 90-degree wrap stays/);
  ...
  assert.doesNotMatch(material.fragmentShader, /neighboring samples can punch triangle-shaped holes/);
  // DO NOT PAINT ON NON CAMERA FACING SIDES.
  // Visibility cannot be repaired by borrowing neighboring samples. Neighbor
  // samples are alpha-only and may run only after the center fragment already
  // matched the current visible depth/normal surface.
  ...
  assert.doesNotMatch(material.fragmentShader, /float visibleSurfaceGrazingFeatherEnd/);
  ...
  assert.doesNotMatch(material.fragmentShader, /Larger brushes need a broader soft/);
  assert.doesNotMatch(material.fragmentShader, /float visibleSurfaceGrazingEdgeAmount/);
  assert.match(material.fragmentShader, /Zero is the 90-degree camera-facing cutoff/);
  assert.doesNotMatch(material.fragmentShader, /float grazingStart = visibleFacingNormalThreshold \+ 0\.01/);
  assert.doesNotMatch(material.fragmentShader, /float grazingEnd = visibleSurfaceGrazingFeatherEnd\(\)/);
  ...
  assert.doesNotMatch(material.fragmentShader, /float visibleSurfaceSoftEdgeAmount/);
  ...
  assert.match(material.fragmentShader, /normal-angle opacity is what makes side strokes show/);
  assert.match(material.fragmentShader, /soft airbrush[\s\S]*mark comes from brush distance\/hardness\/scatter/);
  assert.match(material.fragmentShader, /return 1\.0/);
  ...
  assert.doesNotMatch(material.fragmentShader, /float grazingFeatherEnd = visibleSurfaceGrazingFeatherEnd\(\)/);
  ...
  assert.match(material.fragmentShader, /Do not apply a normal-angle visible-edge fade/);
  assert.match(material.fragmentShader, /hidden\/back fragments still discard before any paint lands/);
  ...
  assert.doesNotMatch(material.fragmentShader, /if \(visibleSurfaceMatched && useVisibleNormalTexture\)/);
  assert.match(material.fragmentShader, /if \(centerVisibleSurfaceMatched && useVisibleNormalTexture && edgeSoftness > 0\.0\)/);
  assert.doesNotMatch(material.fragmentShader, /float centerGrazingEdgeAmount = visibleSurfaceSoftEdgeAmount\(paintFadeViewNormal, paintGateViewNormal\) \* edgeSoftness/);
  ...
  assert.doesNotMatch(material.fragmentShader, /float softCenterVisibleCoverage = visibleSurfaceSoftAlphaCoverage\(paintFadeViewNormal, paintGateViewNormal\)/);
  assert.match(material.fragmentShader, /visibleSurfaceCoverage = visibleSurfaceSoftAlphaCoverage\(paintFadeViewNormal, paintGateViewNormal\)/);
  ...
  assert.doesNotMatch(material.fragmentShader, /visibleSurfaceCoverage = mix\(1\.0, softCenterVisibleCoverage, centerGrazingEdgeAmount\)/);
  assert.doesNotMatch(material.fragmentShader, /visible-side-only soft edge/);
  assert.doesNotMatch(material.fragmentShader, /Do not feed it a[\s\S]*screen-neighborhood depth\/normal mask/);
  ...
  assert.match(material.fragmentShader, /returns 1\.0/);
  assert.doesNotMatch(material.fragmentShader, /\* edgeSoftness/);
  assert.doesNotMatch(material.fragmentShader, /if \(grazingEdgeAmount > 0\.0\)/);
  ...
  assert.match(material.fragmentShader, /paintGateViewNormal\.z <= visibleFacingNormalThreshold/);
  assert.doesNotMatch(material.fragmentShader, /Use the smoothed paint normal for the fade/);
  ...
  assert.doesNotMatch(material.fragmentShader, /vec2 screenPixel = 1\.0 \/ max\(viewportSize, vec2\(1\.0\)\)/);
  ...
  assert.doesNotMatch(material.fragmentShader, /8-connected screen neighbors/);
  ...
  assert.match(material.fragmentShader, /onto the side\/back at cloth and leg wraps/);
  assert.match(material.fragmentShader, /Soft edge behavior above only reduces alpha/);
  assert.match(material.fragmentShader, /must never turn[\s\S]*an unmatched fragment into painted texture/);
  assert.doesNotMatch(material.fragmentShader, /sampleUv \+ screenPixel/);
  assert.doesNotMatch(material.fragmentShader, /sampleUv - screenPixel/);
  assert.doesNotMatch(material.fragmentShader, /sampleUv \+ vec2\(screenPixel\.x, -screenPixel\.y\)/);
  assert.doesNotMatch(material.fragmentShader, /sampleUv \+ vec2\(-screenPixel\.x, screenPixel\.y\)/);
  ...
  assert.match(normalMaterial.fragmentShader, /geometric front-visible normals/);
  assert.match(normalMaterial.fragmentShader, /not[\s\S]*smoothed vertex normals/);
  assert.match(normalMaterial.fragmentShader, /eligibility authority/);
  assert.match(normalMaterial.fragmentShader, /smoothing it can make a near-wrap\/back/);
  ...
  assert.doesNotMatch(normalMaterial.fragmentShader, /visibleSmoothNormal/);
  // DO NOT PAINT ON NON CAMERA FACING SIDES.
  // DO NOT PAINT ON NON CAMERA FACING SIDES.
