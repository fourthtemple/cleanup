...
test("airbrush WebGL brush shader discards fragments outside an active neighbor mask", () => {
  ...
  assert.match(material.fragmentShader, /float softVisibleEdgeCoverage = min\(grazingAngleCoverage, clusteredCenterEdgeCoverage\)/);
  assert.match(material.fragmentShader, /continuous normal-angle/);
  assert.match(material.fragmentShader, /visible-neighborhood Gaussian/);
