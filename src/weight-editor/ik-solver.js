  ...
  Object.assign(BirdWeightEditor.prototype, {
    ...
    updateIkSettingsControls() {
      ...
      if (this.ikCounterRotation) {
        this.ikCounterRotation.disabled = !enabled || settings.solver !== "smooth";
    ...
    applyIkMove() {
      ...
      if (settings.solver === "smooth") {
        ...
        this.solveIkCcd(chainNames, targetWorld);
      }
    ...
    solveIkSmoothChain(chainNames, targetWorld, settings = this.ikSettingsForChain(chainNames)) {
      ...
      const counterRotation = THREE.MathUtils.clamp(Number(settings.counterRotation) || 0, -1, 1);
    ...
    solveIkCcd(chainNames, targetWorld) {
      const endBone = this.bones.get(chainNames[chainNames.length - 1]);
      if (!endBone) {
        return false;
      }
