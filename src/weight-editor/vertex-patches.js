export function installVertexPatchMethods(BirdWeightEditor, deps) {
  ...
  Object.assign(BirdWeightEditor.prototype, {
    ...
    buildPatch() {
      ...
      const adaptivePoseKeyframes = this.serializePoseKeyframeMap?.(this.adaptivePoseKeyframes) || [];
      const adaptiveGuideKeyframes = this.serializePoseKeyframeMap?.(this.adaptiveGuideKeyframes) || [];
      const adaptiveGuideDeltaKeyframes = this.serializePoseKeyframeMap?.(this.adaptiveGuideDeltaKeyframes) || [];
      const includePoseKeyframes = adaptivePoseKeyframes.length || !this.poseKeyframesGenerated;
      const poseKeyframes = adaptivePoseKeyframes.length ? adaptivePoseKeyframes : includePoseKeyframes ? this.serializePoseKeyframes() : [];
      const poseCurveHandles = !adaptivePoseKeyframes.length && includePoseKeyframes ? this.serializePoseCurveHandles?.() || [] : [];
      const poseKeyframeKinds = !adaptivePoseKeyframes.length && includePoseKeyframes ? this.serializePoseKeyframeKinds?.() || [] : [];
      const patch = {
        version: 1,
      ...
      if (poseKeyframeKinds.length) {
        patch.poseKeyframeKinds = poseKeyframeKinds;
      }
      if (adaptiveGuideKeyframes.length) {
        patch.adaptiveGuideKeyframes = adaptiveGuideKeyframes;
      }
      if (adaptiveGuideDeltaKeyframes.length) {
        patch.adaptiveGuideDeltaKeyframes = adaptiveGuideDeltaKeyframes;
      }
      if (poseKeyframes.length) {
        ...
        patch.poseKeyframeSource = adaptivePoseKeyframes.length || !this.timelineKeysSourceWasAutoGenerated ? "authored" : "solved";
      }
      if (deletedVertices.length) {
        patch.deletedVertices = deletedVertices;
      }
      if (!adaptivePoseKeyframes.length && includePoseKeyframes && this.poseKeyframeMode === "replace") {
    ...
    async applySerializedTexturePaints(entries = [], options = {}) {
    ...
    applyPatchObject(patch, { status = true, errorStatus = "Could not apply weight patch", applyPose = true } = {}) {
      try {
        ...
        const applyAsAdaptivePoseLayer = patch.poseKeyframeSource === "authored" && poseLayer.mode !== "replace";
        if (applyAsAdaptivePoseLayer) {
          this.poseKeyframes.clear();
          this.poseCurveHandles?.clear?.();
          this.poseKeyframeKinds?.clear?.();
          this.adaptivePoseKeyframes = this.serializedPoseKeyframeMap?.(poseLayer.keyframes) || new Map();
          this.adaptiveGuideKeyframes = this.serializedPoseKeyframeMap?.(patch.adaptiveGuideKeyframes || []) || new Map();
          this.adaptiveGuideDeltaKeyframes = this.serializedPoseKeyframeMap?.(patch.adaptiveGuideDeltaKeyframes || []) || new Map();
          this.poseKeyframeMode = "additive";
          this.poseKeyframesGenerated = false;
          this.timelineKeysSourceWasAutoGenerated = false;
        } else {
          this.adaptivePoseKeyframes = new Map();
          this.adaptiveGuideKeyframes = this.serializedPoseKeyframeMap?.(patch.adaptiveGuideKeyframes || []) || new Map();
          this.adaptiveGuideDeltaKeyframes = this.serializedPoseKeyframeMap?.(patch.adaptiveGuideDeltaKeyframes || []) || new Map();
        ...
        this.pendingSerializedTexturePaintsApply = null;
        ...
        if (texturePaints.length && typeof this.applySerializedTexturePaints === "function") {
          const textureApply = this.applySerializedTexturePaints(texturePaints, {
            ...
            if (this.pendingSerializedTexturePaintsApply === textureApply) {
              this.pendingSerializedTexturePaintsApply = null;
            }
          });
          this.pendingSerializedTexturePaintsApply = textureApply;
          void textureApply;
        }
        return true;
      } catch (error) {
