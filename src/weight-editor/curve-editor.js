  ...
  Object.assign(BirdWeightEditor.prototype, {
    ...
    setCurveValueAtFrame(frame, value, { sourceFrame = null, rebuild = true, adaptiveAbsolute = false } = {}) {
      ...
      const adaptiveGuideAnchors = useAdaptiveEdit
        ? this.captureAdaptiveEditGuideAnchors?.(targetFrame, [boneName], [channel])
        : null;
      if (useAdaptiveEdit) {
    ...
    deleteCurvePointNearFrame(frame) {
      ...
      const channel = this.curveChannel();
      const targetFrame = Math.max(0, Math.min(this.timelineFrames, Math.round(frame)));
      if (this.adaptiveGuidePointHasChannels?.(targetFrame, boneName, [channel])) {
        this.deleteCurvePointAtFrame(targetFrame);
        return;
      }
    ...
    deleteCurvePointAtFrame(frame, { silent = false, rebuild = true } = {}) {
      ...
      const framePose = this.poseKeyframes.get(targetFrame);
      const bonePose = framePose?.[boneName];
      const frameIsAdaptiveGuide = this.shouldUseAdaptiveEdits?.()
        && (
          this.poseKeyframesGenerated
          || (this.poseKeyframeMode === "replace" && this.timelineKeysSourceWasAutoGenerated)
        );
      const hasAuthoredCurvePoint = bonePose?.[channel] !== undefined
        && !frameIsAdaptiveGuide;
      if (!hasAuthoredCurvePoint) {
        if (!silent) {
          const promotedGuide = this.promoteAdaptiveGuidePointToStatic?.(targetFrame, [boneName], [channel]);
          if (promotedGuide) {
            this.syncPatchJson();
            this.applyPose(this.progress);
            this.syncPoseControlsToCurrentBone();
            ...
            this.setStatus(
              `Deleted frame ${targetFrame} adaptive guide (${promotedGuide.anchors} red neighbor ${promotedGuide.anchors === 1 ? "point" : "points"})`
            );
            return true;
          }
        }
      ...
      this.deleteAdaptiveGuidePoint?.(targetFrame, boneName, [channel]);
